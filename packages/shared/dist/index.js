...
__export(index_exports, {
  DEMO_SYNTHETIC_CANDIDATE: () => DEMO_SYNTHETIC_CANDIDATE,
  DEMO_SYNTHETIC_JOB: () => DEMO_SYNTHETIC_JOB,
...
var createSessionSchema = external_exports.object({
  candidateName: external_exports.string().min(2, "Candidate name must be at least 2 characters").max(100, "Candidate name must be under 100 characters").trim(),
  role: external_exports.string().min(2, "Target role must be at least 2 characters").max(100, "Target role must be under 100 characters").trim(),
  type: external_exports.enum(["technical", "behavioral", "mixed"], {
    errorMap: () => ({ message: "Please select a valid interview type" })
  }),
  durationMinutes: external_exports.coerce.number().refine((val) => [10, 20, 30].includes(val), {
    message: "Duration must be 10, 20, or 30 minutes"
  })
});

// src/index.ts
var DEMO_SYNTHETIC_CANDIDATE = {
